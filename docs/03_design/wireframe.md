...
| 견고성 | Android `contentDescription`, 운영체제 글자 크기 확대, 스크린리더 탐색 순서를 고려한다. |
...
### 7.5 Android Java 구현 기준

- 주요 이미지, 아이콘 버튼, 하단 탭, 퀴즈 보기에는 `contentDescription` 또는 명확한 텍스트 라벨을 제공한다.
...
- `EditText`에는 `inputType`, `imeOptions`, `autofillHints`를 상황에 맞게 지정한다.
...
- 화면은 Android `Activity`와 `res/layout` 기준으로 분리한다.
- 공통 버튼, 입력 필드, 통계 카드, 하단 탭, 로딩 박스, 오류 박스는 `res/drawable`, 공통 스타일, 공통 Java helper 기준으로 재사용한다.
