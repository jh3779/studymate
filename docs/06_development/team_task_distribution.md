...
| 담당 | 파트 | 핵심 책임 | 주요 산출물 |
| --- | --- | --- | --- |
| 우지훈 | PM / UX / 화면 구조 | 화면 흐름, 공통 UI, 라우팅, 발표 문서 | 화면 기본 UI, 공통 위젯, 발표 자료 |
| 최백도 | Firebase / Auth / DB | 인증, Firestore, 데이터 모델, 보안 규칙 초안 | 로그인/회원가입, DB 저장/조회 |
| 윤재이 | AI / 요약 / 프롬프트 | AIService, 요약 생성, 퀴즈 생성 요청, JSON 파싱 | AI 요약/퀴즈 생성 기능 |
| 강도현 | 퀴즈 / 오답 / QA | 퀴즈 풀이, 결과 계산, 오답노트, 통합 테스트 | QuizScreen, 결과 화면, 오답노트 |
...
| 영역 | 1차 담당 | 리뷰 담당 |
| --- | --- | --- |
| 화면 흐름과 공통 UI |  우지훈  | 강도현 |
| Firebase 인증과 Firestore | 최백도 | 우지훈 |
| AI 요청과 JSON 파싱 | 윤재이 | 최백도 |
| 퀴즈 풀이와 오답노트 | 강도현 | 윤재이 |
| 발표와 시연 | 우지훈 | 전체 |
| 통합 테스트 | 강도현 | 전체 |
...
- 공통 모델과 FirestoreService는 여러 파트가 사용하므로 최백도가 인터페이스를 먼저 정리한다.
- AI 응답 형식이 바뀌면 윤재이은 최백도와 강도현에게 즉시 공유한다.
- QuizScreen과 QuizResultScreen은 AI 퀴즈 데이터 구조에 의존하므로 윤재이과 강도현가 함께 테스트한다.
- 8주차에는 신규 기능 추가를 중단하고 버그 수정, UI 정리, 발표 준비에 집중한다.

## 5. 확인 체크리스트

- [ ] 4개 파트의 책임이 겹치지 않는가?
- [ ] 강도현 담당 범위가 실제 팀원 확정 후 바로 넘겨줄 수 있는 수준인가?
- [ ] 각 파트의 담당 파일이 명확한가?
