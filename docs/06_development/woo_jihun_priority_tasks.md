...
| P1 | WJ-006 | SplashScreen UI 및 로그인 상태 분기 연결 지원 | `SplashActivity.java`, `activity_splash.xml` | Auth 담당 작업 | 더미 구현 완료 / Firebase 연동 대기 |
| P1 | WJ-007 | HomeScreen UI 구현 기준 정리 및 연결 지원 | `HomeActivity.java`, `activity_home.xml` | Firestore 조회 함수 | 더미 구현 완료 / Firestore 연동 대기 |
| P1 | WJ-008 | StudyInputScreen, SummaryResultScreen UI 일관성 검수 | 화면 상태, 입력 검증, 로딩 표시 | AI 담당 작업 | 진행 예정 |
| P1 | WJ-009 | QuizScreen, QuizResultScreen UI 일관성 검수 | 보기 선택 상태, 결과 표시 | 퀴즈 담당 작업 | 진행 예정 |
| P1 | WJ-010 | WrongAnswerScreen, MyPageScreen 하단 탭 흐름 검수 | 오답노트, 마이페이지 이동 | 퀴즈/DB 담당 작업 | 진행 예정 |
...
| 핵심 플로우 더미 화면 구현 | 입력, 요약, 퀴즈, 결과, 오답, 마이페이지 Activity 구현 완료 |

## 6. 바로 다음 작업 순서

1. 최백도 담당 Firebase AuthService를 LoginActivity, SignUpActivity, SplashActivity에 연결
2. 최백도 담당 FirestoreService를 HomeActivity 최근 기록/통계에 연결
3. 윤재이 담당 AiService를 StudyInputActivity, SummaryResultActivity에 연결
4. 강도현 담당 QuizService를 QuizActivity, QuizResultActivity, WrongAnswerActivity에 연결
5. `WJ-012` 접근성 수동 점검 기준에 맞춰 주요 화면 검수

현재 저장소에는 Android Java 프로젝트가 생성되어 있으며, 우지훈 담당 화면 흐름 골격은 Java Activity와 XML layout으로 구현되어 있다. Firebase, AI, Firestore 실제 연동은 각 기능 담당자 작업을 연결해야 한다.
...
| SplashActivity | LoginActivity | 비로그인 상태 | 더미 SharedPreferences 사용 중 |
| SplashActivity | HomeActivity | 로그인 상태 | 더미 SharedPreferences 사용 중 |
| LoginActivity | SignUpActivity | 회원가입 링크 클릭 | 가입 성공 후 LoginActivity 복귀 |
| LoginActivity | HomeActivity | 로그인 성공 | 실제 AuthService 연동 예정 |
| HomeActivity | StudyInputActivity | 오늘의 학습 시작 클릭 | 제목, 과목명, 학습 내용 입력 |
| StudyInputActivity | SummaryResultActivity | AI 요약 성공 | 실제 AiService 연동 예정 |
| SummaryResultActivity | QuizActivity | 퀴즈 생성 성공 | 실제 AiService/QuizModel 연동 예정 |
| QuizActivity | QuizResultActivity | 마지막 문제 완료 | correctCount, totalCount extra 전달 |
| QuizResultActivity | WrongAnswerActivity | 오답 상세 보기 클릭 | 실제 WrongAnswerModel 연동 예정 |
| HomeActivity | WrongAnswerActivity | 하단 탭 클릭 | userId 기준 오답 조회 예정 |
...
| 리소스/클래스 | 역할 | 적용 위치 |
| --- | --- | --- |
| BaseActivity.java | 화면 이동, 로그인 더미 상태, 공통 Toast | 모든 Activity |
