...
- AI 기능은 Firebase Functions 배포 URL과 `local.properties`의 `ai.base.url` 설정까지 맞춘 뒤 실제 호출로 검증한다.
- API Key는 소스 코드에 넣지 않는다.
- 퀴즈 JSON 파싱 실패를 반드시 테스트한다.
- 오답 저장은 결과 저장과 함께 트랜잭션으로 처리하고, 동일한 결과 저장을 재시도하면 기존 문서를 갱신해 중복 문서가 생기지 않게 한다.
