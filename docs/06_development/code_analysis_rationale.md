...
퀴즈 화면은 AI가 생성한 `QuizModel` 3개와 4지선다 보기를 기준으로 동작한다. 사용자가 보기를 선택하면 다음 버튼이 활성화되고, 마지막 문제 후 정답 수와 사용자 답안을 결과 화면으로 전달한다.

이렇게 작성한 이유는 다음과 같다.

- 객관식 선택 UI와 선택 상태 표시를 검증하기 위함
- 다음 문제 이동과 진행률 표시를 구현하기 위함
- AI 퀴즈 데이터의 개수와 보기 구성을 화면 진입 전에 검증하기 위함
- 선택된 보기에는 `contentDescription`을 추가해 기본 접근성 상태를 확인하기 위함

현재 구현은 `QuizModel` 목록을 사용하고, 문제/선택지 개수와 정답 인덱스를 검증한 뒤 화면에 표시한다. 화면 회전 시 `currentIndex`, `selectedIndex`, `correctCount`, `attemptId`는 `savedInstanceState`로 복원한다.
...
현재는 오답이 없으면 오답 상세 보기 버튼을 비활성화하고, 오답이 있을 때만 WrongAnswer 화면으로 이동한다. 퀴즈 결과와 오답은 Firestore 트랜잭션으로 저장해 재진입 시에도 중복 문서가 생기지 않도록 처리한다.

### WrongAnswerActivity

오답노트 화면은 틀린 문제의 질문, 내가 선택한 답, 정답, 해설을 보여주는 구조로 작성했다.

현재는 결과 화면에서 전달된 오답 또는 Firestore에 저장된 사용자별 오답 목록을 표시한다. 작성 의도는 다음과 같다.

- 오답 상세 카드의 정보 구조를 먼저 검증하기 위함
- 다시 풀기 버튼과 하단 탭 이동을 연결하기 위함
- `WrongAnswerModel` 목록을 기반으로 내 답, 정답, 해설을 구분해 보여주기 위함

퀴즈 결과 저장 후 오답만 추출해 Firestore에 저장하고, 홈 하단 탭에서는 사용자별 오답 목록을 조회해 표시한다.

### MyPageActivity

마이페이지는 사용자 이메일, 학습 기록 수, 퀴즈 풀이 수, 앱 버전, 로그아웃 버튼을 보여준다.

현재 통계는 Firestore의 학습 기록과 퀴즈 결과를 기준으로 계산하고, 이메일과 로그아웃은 Firebase Auth 상태를 기준으로 처리한다.
...
| --- | --- | --- | --- |
| 인증 | Firebase Auth | 이메일 인증과 로그인 상태 확인 | 실제 기기 QA |
| 회원가입 | Firebase Auth + 사용자 문서 생성 | 가입 흐름 연결 | 예외 메시지 세분화 |
| 홈 데이터 | Firestore 조회 | 최근 학습 기록과 통계 표시 | 빈 상태/오류 상태 수동 QA |
| AI 요약 | `AiService.generateSummary()` | Functions 호출과 응답 검증 | 배포 URL 실환경 확인 |
| 요약 결과 | Intent extra + `noteId` | 저장된 학습 기록으로 퀴즈 생성 | 화면 재생성 대응 |
| 퀴즈 | `QuizModel` 목록 | 3문제/4지선다 검증과 진행 상태 복원 | 실기기 회전 QA |
| 결과 저장 | `QuizResultModel` + `WrongAnswerModel` 트랜잭션 저장 | 결과와 오답 원자적 저장 | 실기기 Firestore 확인 |
| 오답노트 | `WrongAnswerModel` 목록 조회 | 사용자별 오답 상세 표시 | 복습 흐름 확장 |
| 마이페이지 | Firebase Auth + 사용자별 통계 조회 | 이메일/학습 수치 표시 | 로그아웃 수동 QA |

## 7. 실제 구현으로 넘어가기 전 개선해야 할 부분

### 우선순위 1: 데이터 흐름 연결

학습 입력, 요약 결과, 퀴즈, 결과, 오답노트는 모델과 서비스 계층을 통해 연결되어 있다. AI 요약/퀴즈 생성과 저장 상태는 ViewModel에서 유지해 화면 회전 시 중복 호출을 막는다. 이후에는 Intent extra key 상수화와 실패 상태 세분화가 개선 대상이다.

### 우선순위 2: 서비스 계층 추가

Activity가 직접 인증, 저장, AI 요청을 모두 처리하지 않도록 다음 서비스 계층을 둔다.

- `AuthService`
- `FirestoreService`
- `AiService`
- `QuizModel`, `QuizResultModel`, `WrongAnswerModel`

공통 학습 입력 Validator와 AI 작업 상태 보존용 ViewModel을 적용했다. 추후 개선 대상으로는 Intent extra key 상수화와 화면별 상태 객체 통합이 있다.
...
현재 코드는 StudyMate MVP의 입력, 요약, 퀴즈, 결과, 오답, 통계 흐름을 실제 서비스 계층과 연결한 상태다. 남은 핵심 위험은 Firebase Functions 배포 URL, 실기기 인증/권한, 접근성 수동 QA처럼 로컬 코드만으로 확정할 수 없는 항목이다.
