...
현재 구현은 `QuizModel` 목록을 사용하고, 문제/선택지 개수와 정답 인덱스를 검증한 뒤 화면에 표시한다. 이후에는 화면 회전 시 `currentIndex`, `selectedIndex`, `correctCount`가 사라지지 않도록 ViewModel 도입을 검토한다.
...
| 퀴즈 | `QuizModel` 목록 | 3문제/4지선다 검증 | ViewModel 도입 검토 |
...
추후 개선 대상으로는 Intent extra key 상수화, 공통 Validator, 화면 상태 보존용 ViewModel이 있다.
