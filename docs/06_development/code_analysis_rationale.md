...
현재 코드에는 이메일 로그인, 요약 결과, 퀴즈, 오답노트, 마이페이지 통계가 더미 데이터로 들어가 있다. 이는 실제 API가 없기 때문이 아니라 화면 흐름을 먼저 고정하기 위한 의도적인 임시 구현이다.
...
실제 구현 단계에서는 더미 데이터가 남아 있으면 안 되며, 각 TODO 지점은 서비스 계층 호출로 교체해야 한다.
...
현재 화면에는 총 학습 수, 퀴즈 풀이 수, 평균 정답률, 최근 학습 기록이 고정값으로 표시된다. 이 방식은 Firestore 연동 전에 홈 화면의 정보 구조와 하단 탭 이동을 먼저 확인하기 위한 것이다.
...
학습 입력, 요약 결과, 퀴즈, 오답노트가 현재는 각각 더미 데이터로 분리되어 있다. 먼저 화면 간 전달할 key를 `AppConstants`로 정의하고, `StudyNoteModel`, `QuizModel`, `QuizResultModel`, `WrongAnswerModel`을 만들어야 한다.
...
이후 작업의 핵심은 더미 데이터를 제거하고 `model`, `service`, `util` 계층을 추가해 실제 데이터 흐름을 연결하는 것이다. 이 전환이 끝나면 현재 Activity들은 화면 표시와 사용자 이벤트 처리에 집중하고, 실제 비즈니스 로직은 서비스 계층에서 관리하는 구조가 된다.
